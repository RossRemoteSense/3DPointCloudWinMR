			...
			CGPROGRAM
				...
				GS_INPUT VS_Main(VS_INPUT v)
				{
					...
					if (
						...
						//normalisedPosition.z >(_MaxZ - epsilon)  ||
						 isFiltered
						)
					{
						o.color.w = 0;
				...
				void GS_Main2(point GS_INPUT p[1], inout TriangleStream<FS_INPUT> triStream)
				{
					...
					float3 up = float3(0, 1, 0);// UNITY_MATRIX_IT_MV[1].xyz;
					float3 right = float3(1, 0, 0); -UNITY_MATRIX_IT_MV[0].xyz;
				...
				else return input.color;// float4(input.color.x - dt * 0.15, input.color.y - dt * 0.15, input.color.z - dt * 0.15, input.color.w);
				}
